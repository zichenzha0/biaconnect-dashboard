          ...
          <tbody className="divide-y divide-gray-100">
            {shown.map((row, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-1.5 tabular-nums text-right text-gray-700">
                  {row.backspace_ratio != null ? `${(row.backspace_ratio * 100).toFixed(1)}%` : '—'}
                </td>
                <td className="px-3 py-1.5 tabular-nums text-right text-gray-700">
                  {row.n_accelerations > 0 ? row.n_accelerations : '—'}
                </td>
                <td className="px-3 py-1.5 text-center">
                  {row.has_biaffect_for_bin === 1
                    ? <span className="text-indigo-600 font-bold text-xs">✓</span>
                    : <span className="text-gray-200">—</span>}
                </td>
                <td className="px-3 py-1.5 text-center">
                  {row.has_self_report_for_day === 1
                    ? <span className="text-blue-600 font-bold text-xs">✓</span>
                    : <span className="text-gray-200">—</span>}
                </td>
                <td className="px-3 py-1.5 tabular-nums text-right">
                  {row.suicidality_0_9 != null ? (
                    <span className={`font-medium ${row.suicidality_0_9 >= 5 ? 'text-rose-600' : row.suicidality_0_9 >= 2 ? 'text-amber-600' : 'text-gray-700'}`}>
                      {row.suicidality_0_9}
                    </span>
