...
export default function ParticipantFilterPanel({
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="mb-3 space-y-2">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={form.require_self_report}
              onChange={e => set('require_self_report', e.target.checked)}
              className="h-3.5 w-3.5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
            <span className="text-sm text-gray-700">Require self-report data</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox"
      ...
      {dataSource === 'csv' ? (
        <div className="rounded bg-emerald-50 border border-emerald-200 p-2.5">
          <p className="text-xs text-emerald-700 leading-snug">
            <span className="font-semibold">Local real-data mode:</span>{' '}
            filters apply to loaded CSV outputs generated by the Python pipeline.
          </p>
        </div>
      ) : dataSource === 'mock' ? (
        <div className="rounded bg-amber-50 border border-amber-200 p-2.5">
          <p className="text-xs text-amber-700 leading-snug">
            <span className="font-semibold">Mock demo mode:</span>{' '}
            filters apply to synthetic demo data.
          </p>
        </div>
      ) : null}
    </div>
